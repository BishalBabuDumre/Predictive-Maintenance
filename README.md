...
## Feature Engineering & Multi-Scale Signal Extraction

To build an anomaly detection algorithm resilient to both complex atmospheric patterns and hardware failures, the raw temperature data stream was expanded into a **22-dimensional multi-scale feature search-space** (main/training/feature_engineering.py). Rather than relying on a single absolute data point, this framework treats temperature as a continuous physical signal, breaking it down into distinct temporal frequencies, velocities, and operational indicators:

* **Astronomical and Cyclical Hooks:** Raw time stamps are transformed into cyclical sine and cosine representations for hourly (24h), monthly (12m), and seasonal (365.25d) cycles. This eliminates boundary discontinuities (e.g., mapping hour 23 smoothly to hour 0) and forces the model to inherently learn diurnal and seasonal thermal expectations.
* **Multi-Resolution Rolling Windows (3h, 6h, 24h, 7d):** Moving averages and volatilities (standard deviations) are calculated across four distinct time scales. The **3-hour window** captures rapid micro-shocks and immediate atmospheric transitions. The **6-hour window** acts as a low-pass stabilizer, capturing diurnal trend phases while filtering out brief transient noise (like temporary cloud cover). The **24-hour window** establishes day-versus-night baselines and isolates local expectations via a 24-hour deviation feature (`dev_24h`). The **7-day macro window** acts as a long-term memory buffer, capturing regional climate regimes.
* **Hierarchical Kinematics (Slopes & Accelerations):** First and second derivatives are extracted across multiple horizons to hand the model the exact structural trajectory of the signal. First derivatives (slopes) capture micro, meso, and macro thermal velocities. Second derivatives (accelerations) isolate rapid compounding changes. A high 3-hour acceleration flags intense weather fronts or sudden hardware shocks; a 24-hour acceleration tracks compounding regional weather anomalies; and a 7-day acceleration serves as the ultimate early-warning indicator for exponential, runaway hardware component degradation.
* **Digital Integrity Checks (`repeat_count`):** An exact data duplication tracker counts consecutive static values over a sliding 6-hour window. Because open-air ambient temperatures naturally fluctuate due to thermal micro-noise, a pegged maximum `repeat_count` indicates an absolute flatline anomaly, allowing the model to instantly distinguish between a true stagnant environmental condition and a digital data-stalling or register freeze fault.
* **Target Regularization (`target_delta`):** The network's training objective is set to predict the next-step change in temperature ($\Delta T$) rather than the raw absolute value. This target choice forces the underlying architecture to map genuine atmospheric transitions and thermal dynamics, completely mitigating the risk of structural identity laziness or posterior collapse.

By fusing these multi-tiered features, the final algorithm achieves high operational stability—capable of elegantly isolating transient environmental anomalies (such as sudden weather fronts) from insidious, long-term hardware calibration drift and systematic sensor bias.
...
The following terminal-style histogram captures the exact distribution of Absolute Errors ($AE$) generated by the VAPA model over **3,103 clean test data samples**. This underlying distribution directly validates the structural observations behind the model's low RMSE, tight 95% quantile, and bounded maximum error. You can see the heavy concentration of mass in the first few bins—over 96% of all test points experience an error of less than 3.88°C, showcasing the highly predictable nature of the VAPA model.
